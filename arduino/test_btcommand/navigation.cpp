...
void move_to_position(int target_x, int target_y) {
    ...
    mySerial.print(target_x);
    mySerial.print(",");
    mySerial.print(target_y);
    mySerial.println(")");
...
void send_current_state() {
    ...
    Serial.print(" dir=");
    Serial.println(current_state.direction);

    // 2. direction 범위 검증
    if (current_state.direction < 1 || current_state.direction > 4) {
        Serial.print("dir err: ");
        Serial.println(current_state.direction);
        current_state.direction = NORTH;  // 기본값으로 설정
        Serial.println("dir reset NORTH");
    }
    ...
    default:
        direction_char = '?';
        Serial.print("unknown dir: ");
        Serial.println(current_state.direction);
        break;
    }

    // String 객체 대신 직접 출력으로 메모리 절약
    mySerial.print(current_state.x);
    mySerial.print(',');
    mySerial.print(current_state.y);
    mySerial.print(',');
    mySerial.println(direction_char);
